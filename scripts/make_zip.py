...
def  main():
  ...
  installer = "\installer\FacadeTest Installer.exe"
   
  if demo:
    installer = "\installer\FacadeTest Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\FacadeTest manual.pdf" 
  ]
  ...
  ZIPNAME = "FacadeTest-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "FacadeTest-v" + FULLVERSIONSTR + "-win-demo.zip"
